...
for directory in data/fanniemae/????Q?; do
  [[ $directory =~ $yq_regex ]]
  year=${BASH_REMATCH[1]}
  ...
  echo $fannie_loans_file
  echo $fannie_monthly_file
...
fannie_mapping_file="data/fanniemae/HARP_Files/Loan_Mapping.txt"
...
if [ -f $fannie_mapping_file ]; then
  ...
  fannie_loans_file="data/fanniemae/HARP_Files/Acquisition_HARP.txt"
  fannie_monthly_file="data/fanniemae/HARP_Files/Performance_HARP.txt"
